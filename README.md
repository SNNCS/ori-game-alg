...
This repository implements a differentiable cognitive architecture for
declarative game environments. The system closes the loop from latent action
generation to interpretation, future simulation, action choice, observation,
and learning.
...
generated by the agent, then `GenericGameAdapter` grounds that vector through a
`GameSpec` into an executable `GroundedAction`. In the Ultimatum spec, that
grounded intervention is a continuous kept-share bid; other specs can expose
continuous, binary, or categorical controls.

Concrete game scenarios are described with Python dataclass specs rather than
new handwritten adapters. Built-in specs include Ultimatum, Prisoner's Dilemma,
Chicken, Stag Hunt, Public Goods, and First-price Auction.
...
each generated intervention against adapter-provided possible world responses.
Continue branches remain leaves unless an adapter explicitly provides a
continuation action hook, so the core planner does not invent another agent's
strategy.
...
accordingly, preserving coherent probability mass whenever an adapter supplies
...
vectors. `GenericGameAdapter` decodes those vectors into continuous, binary, or
categorical action controls declared by the active `GameSpec`.
...
1. Runtime observations are encoded into an observation-derived `BeliefState`.
2. Latent action vectors are generated from current situation and relation
   state.
3. The adapter decodes latent actions into executable interventions.
4. Each candidate intervention is encoded as signal `s`.
5. Each observer interprets that action through its own relation edge and
   situation state.
6. The interpretation engine produces the action-conditioned intent matrix `Z`.
7. The responder's intent drives a generated future outcome tree.
8. Path dependence adjusts branch probabilities from recent history.
9. Tree metrics produce differentiable value signals.
10. A decision engine chooses the best action/signal intervention.
11. Runtime learning separates `ActionEvent`, `WorldResponse`, `Outcome`,
    realized utility, trajectory return targets, and policy credit assignment.
...
| [game_spec.py](game_spec.py) | GameSpec dataclasses, expression DSL, effects, and GroundedAction. |
| [generic_adapter.py](generic_adapter.py) | Generic GameSpec interpreter and adapter contract. |
| [runtime.py](runtime.py) | RuntimeSnapshot, Observation, ActionEvent, WorldResponse, transitions, and schema checks. |
| [belief.py](belief.py) | Explicit BeliefState produced from observations and interpreted actions. |
| [action_model.py](action_model.py) | Mixed action policy output and adapter-grounded candidate interventions. |
| [specs/](specs) | Built-in declarative game specs. |
| [decision.py](decision.py) | Candidate interventions, predicted futures, scoring, and action selection. |
| [trajectory.py](trajectory.py) | Trajectory steps, return targets, and learning coordination. |
| [experience.py](experience.py) | Raw outcomes, realized utility, and single-step loss helpers. |
| [future_tree.py](future_tree.py) | World-response model, counterfactual planner, future value, and path dependence. |
...
print(action_event.label)
print(decision.selected.latent_action.shape)
print(decision.selected_future.metrics)
```

To run another built-in game:

```python
from agent import CognitiveAgent
from generic_adapter import GenericGameAdapter
from specs import PRISONERS_DILEMMA_SPEC

agent = CognitiveAgent(adapter=GenericGameAdapter(PRISONERS_DILEMMA_SPEC))
snapshot = agent.runtime_snapshot()
